...
jQuery(document).ready(function($){
	var reccocmmloadInc = 3;
	$('#ShopCommentAddButt').click(function(){
		var pID = $(this).attr('data-postid');
		$('#ReccommendPostBoxLoading').css('display', 'block');
		$.ajax({
			url:  ajaxURL,
			type: 'POST',
			data: {
				action:     'shopcomloadmo',
				loadPstart: reccocmmloadInc,
				postID:     pID,
			},
			success: function(data){
				if(data.length > 0){
					$('#ShopCommentAddBox').append(data);
					reccocmmloadInc = reccocmmloadInc + 3;
				}else{
					$('#ShopCommentAddButt').css('display', 'none');
				}
				$('#ReccommendPostBoxLoading').css('display', 'none');
			}
		});
	});
});

//===ADD or DELETE virtual product from cart && reload cart===
jQuery(document).ready(function($){
	//===ADD VIRTUAL PROD===
	$('.prodAddDellToCart').click(function(){
		$('#siteTopCart').html('loading...');
		var pID = $(this).attr('data-prodid');
		var par = $(this).attr('data-parentid');
		$.ajax({
			url:  ajaxURL,
			type: 'POST',
			data: {
				action: 'cartadddellprod',
				prodID: pID,
			},
			success: function(data){
				data = JSON.parse(data);
				$('#siteTopCart').html(data.content);
				$('.adress-costs__item').removeClass('adress-costs__item-active');
				$('#'+par).addClass('adress-costs__item-active');
				$('#siteTopCartCounter').html(data.icount);
				alert(data.msg);
			}
		});
	});
	$('.siteTopCartItemDelete').click(function(){
		$('#siteTopCart').html('loading...');
		...
		$.ajax({
			...
			success: function(data){
				$('#siteTopCart').html(data);
			}
		});
	});
	
	//===ADD FIZICAL PROD===
	$('.prodAddToCart').click(function(){
		$('#siteTopCart').html('loading...');
		var pID = $(this).attr('data-prodid');
		$.ajax({
			url:  ajaxURL,
...
//===READMO RELATED POSTS===
jQuery(document).ready(function($){
	var reccocmmloadInc = 3;
	$('#ReccommendPostLoadMore').click(function(){
		$('#ReccommendPostBoxLoading').css('display', 'block');
		$.ajax({
			url:  ajaxURL,
			type: 'POST',
			...
			success: function(data){
				if(data.length > 0){
					$('#ReccommendPostBox').append(data);
					reccocmmloadInc = reccocmmloadInc + 3;
				}else{
					$('#ReccommendPostLoadMore').css('display', 'none');
				}
				$('#ReccommendPostBoxLoading').css('display', 'none');
			}
		});
...
$( document ).ready(function() {
    ...
    /* var blockDate = ["2013-03-14","2013-03-15","2013-03-16"]
    ...
    $( ".masters__link" ).on( "click", function( event ) {
